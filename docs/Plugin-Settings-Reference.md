...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| **Refresh the Priority Queue after each session** | Boolean | `true` | When you leave the queue after practising a [Priority Queue](Priority-Review-Document.md) document, drains what you reviewed and tops the document back up to its fill target a couple of seconds later, so it is ready before the next Practice. Never runs while a queue is open, and not in Light Mode, where you refresh from the popup instead. 📖 [Refresh after every session](Priority-Review-Document.md#refresh-after-every-session). |
