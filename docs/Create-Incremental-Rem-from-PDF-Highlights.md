...
- **Pin styling**: The pin itself is drawn with a **hairline ring** at reduced opacity (full strength on hover, or while you edit the Rem), so it reads as a distinct object instead of a stray glyph at the end of a sentence. The ring is deliberately **colourless** — hue already means priority band, `pdfextract` blue or `incremental` green — and has no background fill, so a pin inside a highlighted extract never covers the highlight's own colour. See [Utilities → Reference pins are ringed](Utilities.md#reference-pins-are-ringed).
