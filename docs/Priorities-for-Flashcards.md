...
Flashcard prioritisation is **off by default**. Turn it on in the plugin's settings popup — run **`Incremental RemNote: Settings`** (quick code `ies`) → **Flashcard Prioritisation** → *Enable Flashcard Prioritisation* — then reload RemNote. (Before v1.0.45 this switch lived in RemNote's own plugin settings panel.)
