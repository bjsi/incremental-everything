...
function PriorityShieldGraph() {
  ...
  const numCharts = (effectiveDocScopeId && hasDocData ? 1 : 0) + (hasKbData ? 1 : 0) + (effectiveDocScopeId && hasCardDocData ? 1 : 0) + (hasCardKbData ? 1 : 0);
  ...
  return (
    <div className="p-4 flex flex-col" style={{ width: '950px', height: containerHeight, overflowY: 'auto' }}>
      ...
      {effectiveDocScopeId && hasDocData && renderChart(docChartData!, `📄 ${documentName || 'Document'} IncRem Shield`, '#e74c3c', '#f39c12')}
      ...
      {effectiveDocScopeId && hasCardDocData && renderChart(cardDocChartData!, `🎴 ${documentName || 'Document'} Card Shield`, '#d35400', '#f1c40f')}
