import { ReactRNPlugin, RNPlugin, SelectionType } from '@remnote/plugin-sdk';
...
type CommandDependencies = {
  ...
  removeAllCardPriorityTags: (plugin: RNPlugin) => Promise<void>;
};
...
export async function registerPluginCommands(
  ...
  await plugin.app.registerCommand({
    ...
    action: async () => {
      const rem = await createExtract(plugin);
      if (!rem) {
        return;
      }
  ...
  await plugin.app.registerCommand({
    ...
    action: async () => {
      await deps.removeAllCardPriorityTags(plugin);
    },
  });
...
async function createExtract(plugin: ReactRNPlugin) {
  const selection = await plugin.editor.getSelection();
  if (!selection) {
    return;
  }
...
async function buildOptimizedCache(plugin: RNPlugin) {
  ...
  console.log(
    `CACHE: Total ${uniqueRemIds.length} rems to process (${cardRemIds.length} with cards + ${inheritanceRemIds.length - cardRemIds.length} inheritance-only)`
  );
